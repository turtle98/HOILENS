...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prior=None, query=None,mask = None, la_masks=None):
       ...
       #import pdb; pdb.set_trace()
        #changed
        if query != None:
            x = torch.cat([query.unsqueeze(0), (self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype,device=x.device)) + self.positional_embedding.to(x.dtype)[:1,:].unsqueeze(0), x + self.positional_embedding.to(x.dtype)[1:,:].unsqueeze(0)], dim=1)  # shape = [*, grid ** 2 + 1, width]
        else:
            x = torch.cat([(self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype,device=x.device)) + self.positional_embedding.to(x.dtype)[:1,:].unsqueeze(0), x + self.positional_embedding.to(x.dtype)[1:,:].unsqueeze(0)], dim=1)  # shape = [*, grid ** 2 + 1, width]
        x = self.ln_pre(x)
        x = x.permute(1, 0, 2)  # NLD -> LND
        intermediates = []
        for rn, rl in enumerate(self.transformer.resblocks):
            x = rl(x,mask,prior,la_masks)
            if (rn == len(self.transformer.resblocks)-2):
        ...
        # also convert intermediates to shape [bs, seq_len, width], optionally apply ln_post and proj if needed

        # example: convert intermediates to list of [bs, seq_len, width]
        intermediates = [layer_out.permute(1, 0, 2) for layer_out in intermediates]

        return x[:,:-576,:], x[:,-576:,:].view(bs,h//self.patch_size, w//self.patch_size, -1).permute(0, 3, 1, 2), intermediates
...
def build_model(state_dict: dict, use_adapter=True, adapter_pos='all',args=None):
    ...
    model = CLIP(
        ...
        adapter_layers=adapter_layers, args=args
    )
