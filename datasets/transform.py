...
class Expand2Square(object):
    def __init__(self, background_color=(0, 0, 0)):
        """
        background_color: tuple of ints (R, G, B), e.g. (int(0.481*255), int(0.457*255), int(0.408*255))
        """
        self.background_color = tuple(int(x*225) for x in background_color)

    def __call__(self, img, target=None):
        """
        img: PIL.Image
        target: Optional dict with bounding boxes etc. (if needed, you can update boxes here)

        Returns:
            padded_img: PIL.Image
            target: unchanged or modified if you implement bbox update
        """
        ...
        if width > height:
            new_img = Image.new(img.mode, (width, width), self.background_color)
            new_img.paste(img, (0, (width - height) // 2))
        else:
            new_img = Image.new(img.mode, (height, height), self.background_color)
            new_img.paste(img, ((height - width) // 2, 0))

        # TODO: optionally update target boxes here to adjust for padding offsets
