...
from models.ours_best import build_detector
...
def compute_throughput_cuda1(name, model,  device, batch_size, resolution=336):
        model.eval()
...
def main(rank, args):
    ...
    def worker_init_fn(worker_id):
        # PyTorch generates the worker seed internally like this:
        worker_seed = torch.initial_seed() % 2**32
        # np.random.seed(worker_seed)
        # random.seed(worker_seed)
        
        # Save/log the seed
        per_worker_seeds[worker_id] = worker_seed
        print(f"[a6000 Worker] ID: {worker_id}, Seed: {worker_seed}")
    ...
    for n, p in lain.clip_head.named_parameters():
        p.requires_grad = False
        if args.baseline:
            for n, p in lain.clip_head.linear_shortcut.named_parameters():
