...
class LAINadapter(nn.Module):
    def __init__(self,
        ...
        if adapter_scalar == "learnable_scalar":
            self.scale = nn.Parameter(torch.ones(1024) * 1e-9)
            self.scale2 = nn.Parameter(torch.ones(1024) * 1e-9)
        else:
            self.scale = float(adapter_scalar)

        self.alpha = args.adapter_alpha

        self.down_proj = nn.Linear(1024, self.down_dim)
        self.non_linear_func = nn.LeakyReLU()

        self.down_proj2 = nn.Linear(1024, self.down_dim)

        self.up_proj = nn.Linear(self.down_dim * 2, self.up_dim)
        self.up_proj2 = nn.Linear(self.down_dim, self.up_dim)

        self.down_proj3 = nn.Linear(1024, self.down_dim)
    ...
    def forward(self, x, prior=None, la_masks=None):
        ho_tokens, im_tokens = x[:-576], x[-576:]
        ...
        im_tokens_down = self.alpha * im_tokens_down.view(24, 24, -1,self.down_dim).permute(2, 3, 0, 1) + prior.unsqueeze(0).permute(0, 3, 1, 2)
        ...
        object_boxes = torch.cat([object_boxes, torch.tensor([[0, 0, 336, 336]]).cuda()], dim=0)  # human
        object_feats = torchvision.ops.roi_align(la_im_tokens.permute(1, 2, 0).view(-1, 1024, 24, 24), [object_boxes],
                                                 output_size=(7, 7),
