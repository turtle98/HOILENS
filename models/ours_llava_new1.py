 ...
 #/  self.lm_head_embeddings.pow(2).mean(-1, keepdim=True).add(1e-6).sqrt()
    ...
    def compute_sim_scores(self, region_props: List[dict], image, targets, priors=None):
        ...
        for b_idx, props in enumerate(region_props):
            ...
            # h_feats0 = torchvision.ops.roi_align(llava_feat_for_roi, roi_boxes,
            #                                         output_size=(7, 7),
        ...
        #     o_feats0 = o_feats0.squeeze(0)[o_inverse_indices].unsqueeze(0)
            ...
            ho_out = self.cross_attend_lora_o(ho_detr_feats.unsqueeze(0), llava_features, ) 
            ...
            # global_verb_logits = asd1[self.verb_token_ids]
            ...
            #h_vocab = attn_weights_h.squeeze(0) @ lm_head_t
            ...
            logits_ho = self.verb_projection_ho(normed_ho) - normed_ho @ self.mu.T #- global_logit
            logits_h = self.verb_projection_ho(normed_h)- normed_h @ self.mu.T  #- global_logit
            logits_o = self.verb_projection_ho(normed_o) - normed_o @ self.mu.T #- global_logit
            ...
            logits = (logits_ho + logits_h +logits_o)/3# / #100 # / (w_ho + w_h + w_o)
        ...
        #     logits1 = h_feats_stacked @ self.lm_head_embeddings.T.to(hidden_states.device).to(torch.bfloat16)
            ...
            #print(self.clip_head['tokenizer'].decode(torch.topk(ho_vocab[4], 30, dim=-1)[1]))
            ...
            # print(self.clip_head['tokenizer'].decode(torch.topk(h_vocab[0], 200, dim=-1)[1]))
            import pdb; pdb.set_trace()
