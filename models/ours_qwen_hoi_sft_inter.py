...
class HOIQWEN_SFT(nn.Module):
    ...
    def compute_sim_scores(
        ...
        for b_idx, props in enumerate(region_props):
            ...
            # ------------------------------------------------------------------
            # Stage 1 early exit: compute inter_loss only, skip LLM forward
            # ------------------------------------------------------------------
            if self.training_stage == 1 and self.training:
                inter_logits = self.interactiveness_head(
                    ho_queries.squeeze(0).float()
    ...
    def forward(
        ...
        logits, prior, bh, bo, objects, gen_losses, inter_logits, inter_losses = self.compute_sim_scores(
            region_props, images_clip, targets, text_labels=text_labels,
        )

        # prior, bh, bo, objects, inter_logits, inter_losses = self.compute_sim_scores(
        #     region_props, images_clip, targets, text_labels=text_labels,
        # )
        ...
        if self.training:
            # ------------------------------------------------------------------
            # Stage 1: only inter_loss (LLM was skipped in compute_sim_scores)
            # ------------------------------------------------------------------
            if self.training_stage == 1:
                if inter_losses:
                    inter_loss_sum   = sum(inter_losses)
                    inter_loss_count = torch.tensor(
                        sum(t.numel() for t in inter_logits), dtype=torch.float, device=device
                    )
                    if dist.is_available() and dist.is_initialized():
                        world_size = dist.get_world_size()
                        dist.barrier()
                        dist.all_reduce(inter_loss_count, op=dist.ReduceOp.SUM)
                        inter_loss_count = inter_loss_count / world_size
                    total_loss = inter_loss_sum / inter_loss_count.clamp(min=1)
                else:
                    # No valid pairs — keep grad alive through the head params
                    total_loss = sum(p.sum() for p in self.interactiveness_head.parameters()) * 0.0
        ...
        return self.postprocessing(boxes, bh, bo, logits, prior, objects, inter_logits, image_sizes)

    def postprocessing(self, boxes, bh, bo, logits, prior, objects, inter_logits, image_sizes,):
        n           = [len(b) for b in bh]
        logits      = torch.cat(logits).split(n)
        inter_logits = [il.float() for il in inter_logits]

        detections = []
        for bx, h, o, lg, pr, obj, il, size in zip(
            boxes, bh, bo, logits, prior, objects, inter_logits, image_sizes
        ):
            pr = pr.prod(0)
            x, y = torch.nonzero(pr).unbind(1)
            iscores = torch.sigmoid(il)          # [n_pairs]  interactiveness prob
