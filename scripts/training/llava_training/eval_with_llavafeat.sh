...
CUDA_VISIBLE_DEVICES=0 torchrun --rdzv_id $id --rdzv_backend=c10d --nproc_per_node=$gpu_num --rdzv_endpoint=127.0.0.1:$port \
         main_llava_training.py --pretrained checkpoints/pretrained_detr/detr-r50-hicodet.pth --clip_dir_vit checkpoints/pretrained_clip/ViT-L-14-336px.pt --output-dir /hub_data1/taehoonsong/LAIN/checkpoints/CVPR/VKD/no_text_tokens/NRF/30/epoch10_lr1e3_drop5_addzsetotraining_noscale_changebacktoours_objnorm_batch8_2gpu_1106 \
         --dataset hicodet --zs --zs_type unseen_object --num_classes 117 --num-workers 4 \
         --epochs 10 --lr-head 1e-3 --lr-drop 10 \
         --adapt_dim 128 --batch-size 8 \
         --print-interval 100 --layer 20 --eval --zse --cse --resume /hub_data1/taehoonsong/LAIN/checkpoints/CVPR/orig_llava/no_text_tokens/1108/UO/30/epoch15_lr1e3_drop10_noscale_klzse05_backtoours_objnorm_batch8_2gpu/ckpt_21417_11.pt


# CUDA_VISIBLE_DEVICES=0,4 torchrun --rdzv_id $id --rdzv_backend=c10d --nproc_per_node=$gpu_num --rdzv_endpoint=127.0.0.1:$port \
#          main_llava_training.py --pretrained checkpoints/pretrained_detr/detr-r50-hicodet.pth --clip_dir_vit checkpoints/pretrained_clip/ViT-L-14-336px.pt --output-dir /hub_data1/taehoonsong/LAIN/checkpoints/CVPR/orig_llava/no_text_tokens/UO/30/epoch10_lr1e4_drop5_addzsetotraining_noscale_changebacktoours_objnorm_batch8_2gpu_1106 \
#          --dataset hicodet --zs --zs_type unseen_object --num_classes 117 --num-workers 4 \
