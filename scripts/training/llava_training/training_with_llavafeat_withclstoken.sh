...
CUDA_VISIBLE_DEVICES=4,6 torchrun --rdzv_id $id --rdzv_backend=c10d --nproc_per_node=$gpu_num --rdzv_endpoint=127.0.0.1:$port \
         main_llava_training.py --pretrained checkpoints/pretrained_detr/detr-r50-hicodet.pth --clip_dir_vit checkpoints/pretrained_clip/ViT-L-14-336px.pt --output-dir /hub_data1/taehoonsong/LAIN/checkpoints/CVPR/orig_llava/no_text_tokens_crossattn/UV/30 \
         --dataset hicodet --zs --zs_type unseen_verb --num_classes 117 --num-workers 4 \
         --epochs 25 --lr-head 1e-3 --lr-drop 20 \
         --adapt_dim 128 --batch-size 8 \
         --print-interval 100  #--resum4 /hub_data1/taehoonsong/LAIN/checkpoints/rebuttal_a6000ada/orig_llava_uv_layer30_epoch10_drop5_1e3_2gpu8/ckpt_22220_10.pt 
